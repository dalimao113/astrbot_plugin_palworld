...
COMMANDS: list[CommandSpec] = [
    ...
    CommandSpec("配出谁", "_cmd_breed_out", ("能配谁", "能配出谁", "当亲代", "作为亲代", "正向配种", "breedout"), cooldown=True, pass_args=True, description="某帕鲁作为亲代能配出的全部后代", category="paldex"),
    CommandSpec("配种榜", "_cmd_breed_rank", ("配种排行榜", "配种排行", "能配榜", "breedrank"), cooldown=True, pass_args=True, description="配种榜:能配出后代最多的帕鲁排行", category="paldex"),
    CommandSpec("配工种", "_cmd_breed_worksuit", ("配工作帕鲁", "按工种配", "工种配种", "worksuitbreed"), cooldown=True, pass_args=True, description="通用查询:满足某工作适性等级的帕鲁 + 各自配种组合", category="paldex"),
    CommandSpec("我可以配工种", "_cmd_my_breed_worksuit", ("我能配工种", "我配工种", "我可以配", "mybreedwork"), cooldown=True, pass_args=True, description="用我现有帕鲁规划配出某工作适性等级的帕鲁", category="player"),
