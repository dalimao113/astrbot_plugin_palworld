...
- **二次确认**：封禁/关服/重启/回档/重置存档/恢复存档 等高危操作发起后，需管理员在超时时间内回复「帕鲁 确认」才执行，超时作废。
...
- **运维模式**：以**只读**（`:ro`）挂载 `docker.sock`。启用容器负载显示、存档解析、以及关服/重启/回档/重置/恢复等运维能力。仅在你信任运行环境时启用。
...
| 临时文件 | 拉取的存档临时目录用后即删；本地渲染的临时 PNG 定期清理 | `services/save_service.py`、`render/renderer.py` |
