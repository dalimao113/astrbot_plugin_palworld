...
| `/帕鲁小队进度` | 按群聚合已绑定队员的**287 图鉴解锁/152 传送点/8 高塔/5 剧情 Boss/122 头目/407 遗物/123 区域**进度；全部按当前 build 稳定 ID 求交集，旧版本残留标记不再造成“全部完成”。目录或单项存档不可核验时只显示已记录数/存档不可用，不伪造总量、剩余和完成状态；地牢仅显示普通/固定累计清理次数及 170 个入口目录，无法计算逐入口完成度。|
...
| `update_check_hours` | `1` | Steam 服务端版本检测间隔（小时，最小 1 小时；失败后 10 分钟重试）|
