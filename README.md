...
![version](https://img.shields.io/badge/version-1.47.0-6366F1?style=flat-square)
...
| `notify_server_update` | `true` | 检测到服务端更新后通知 QQ、自动发送本次完整更新公告，并在镜像计划维护前约 5 分钟发送游戏公告 |
...
- **游戏更新预告**：默认每 1 小时检查一次 Steam 服务端 manifest，临时失败会在 10 分钟后重试；识别到镜像的每小时计划后，检测会自动对齐到更新倒计时前沿（默认整点检测、每小时第 5 分钟由镜像检查更新）。发现更新后先通知 QQ，紧接着自动发送 Steam 官方简体中文完整更新公告，长文使用合并转发并保留原文链接；部分群发送失败时只在 10 分钟后补发失败群。镜像更新倒计时窗口还会向游戏内发送中文公告，需要更新且有玩家在线时倒计时 5 分钟。实际更新和重启仍由镜像的 `AUTO_UPDATE_ENABLED` / `UPDATE_ON_BOOT` 负责，插件不会自行绕过确认重启 Docker。
