...
| `/帕鲁重启服务器` | 存档后重启服务器，约 1 分钟恢复、世界不变；前置存档失败则中止且不停机（**需二次确认**；也可 `/帕鲁重启`） |
...
| `/帕鲁小队进度` | 按群聚合已绑定队员的**图鉴/传送点/8 高塔/5 剧情 Boss/125 头目/155 遗物/123 区域**当前、总量与还差数量，并显示地牢普通/固定累计次数、当前地图 170 个入口和各自**下一步任务**（存档只读自动同步；地牢存档没有逐入口完成集合，不伪造剩余数）|
...
| `data/progress_catalog.json` | JSON 数据 | 小队图鉴、传送点、高塔、剧情 Boss、头目、地牢、遗物与区域的当前 build 总量及稳定 ID |
