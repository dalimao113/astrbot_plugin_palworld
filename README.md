...
![version](https://img.shields.io/badge/version-1.46.0-6366F1?style=flat-square)
...
| `/帕鲁配出谁 <帕鲁>` | 正向展开：某帕鲁作为亲代 × 各搭档能配出的全部后代（按稀有度排序，也可 `/帕鲁能配谁`）|
| `/帕鲁配种榜` | 配种榜：按每只帕鲁能配出的后代种类数排行，越多越靠前（也可 `/帕鲁配种排行`）|
| `/帕鲁配工种 <工种> [等级]` | 通用查询：列出所有满足「工种≥等级」的帕鲁 + 各自的配种组合（按工种等级降序，翻页，也可 `/帕鲁按工种配`）|
| `/帕鲁我可以配工种 <工种> [等级]` | 用你帕鲁箱现有帕鲁规划配出满足「工种≥等级」的帕鲁（配种链，步数最少）；已有也继续给配种链；配不出则通用最短链（也可 `/帕鲁我能配工种`）|
...
| `/帕鲁塔主 [名]` | 7 大高塔塔主：属性/等级/血量/**基础攻防/弱点属性/主要技能**/所在/掉落 + 克制提示（也可 `/帕鲁高塔`）|
| `/帕鲁突袭 [名]` | 突袭 Boss（贝菈诺娃等）：等级/血量/**基础攻防/弱点/技能**/掉落/获取方式与打法提示（也可 `/帕鲁boss`）|
...
| `/帕鲁用途 <材料> [页]` | **材料用途反查**（材料路线的逆）：这材料是哪些物品/建筑的配方原料 + 各需多少；用途多的自动分页（也可 `/帕鲁材料用途`）|
| `/帕鲁种属 [人形/四足兽/鸟类/鱼类/龙类/其它]` | 按**种属**分类浏览图鉴：无参给分类菜单（各类数量），带种属出网格（真实立绘）；支持别名（龙/四足/鱼…）|
| `/帕鲁科技树 [等级]` | 建造/科技**解锁路线图**：无参给各等级解锁总览（含古代科技标注），带等级列该级全部解锁（真实图标+技术点）；古代科技单独标注（也可 `/帕鲁解锁路线`）|
| `/帕鲁牧场 [产物]` | **牧场产出总览**：有牧场适性的帕鲁 → 各自产物（真实立绘+物品图标）；带产物名反查谁产它（如 `/帕鲁牧场 羊毛`）；挖掘类如实标「随机道具」（也可 `/帕鲁放牧`）|
| `/帕鲁地图收集 [类别]` | 以世界地图为底图，把**从游戏关卡对象提取的全量坐标点**标到图上（编号+名称+坐标图例）：📍传送点（138）/🗿遗物雕像（142）/🕳️地牢入口（170）/禁猎区/遗迹遗址/聚落/洞窟/孤岛；无参给类别菜单（也可 `/帕鲁地标`）|
| `/帕鲁突变` | 1.0 **突变机制**与特殊蛋糕说明（也可 `/帕鲁突变配种`、`/帕鲁突变系统`）|
| `/帕鲁植入体 [名]` | **植入体图鉴**（1.0 改造系统）；无参给总览、带名给详情（也可 `/帕鲁改造`）；按编号查用 `/帕鲁植入体查询 <编号>`（也可 `/帕鲁植入体编号`）|
| `/帕鲁研究所 [项]` | **研究所图鉴**（1.0）（也可 `/帕鲁研究`、`/帕鲁实验室`）|
| `/帕鲁词条大全 [页]` | **全部被动词条分类查询**（翻页；也可 `/帕鲁词条图鉴`、`/帕鲁全部词条`）|
| `/帕鲁哪里掉 <物品>` | **掉落查询**：某物品由哪些帕鲁/来源掉落（也可 `/帕鲁掉落`、`/帕鲁爆什么`）|
| `/帕鲁怎么配 <子代>` | **配种路线**：配出某帕鲁的配种链（也可 `/帕鲁配种路线`、`/帕鲁配种链`）|
| `/帕鲁症状 [症状]` | **伤病治疗查询**：帕鲁各症状如何治疗（也可 `/帕鲁治疗`、`/帕鲁怎么治`）|
| `/帕鲁热力` | 服务器**在线热力图**（在线时段分布；也可 `/帕鲁热力图`、`/帕鲁在线热力`）|
| `/帕鲁更新公告` | 服务器/游戏**更新公告**（也可 `/帕鲁更新日志`、`/帕鲁补丁说明`）|
| `/帕鲁战力榜` | 帕鲁**战力等级排行**（图鉴向，按种族基础战力；也可 `/帕鲁战力`、`/帕鲁最强帕鲁`）|
| `/帕鲁玩家战力榜` | 全服玩家**拥有/抓捕帕鲁的战力榜**（也可 `/帕鲁全服战力榜`、`/帕鲁服务器战力榜`）|
| `/帕鲁资产榜` | 玩家**资产/身价排行**（也可 `/帕鲁身价榜`、`/帕鲁财富榜`、`/帕鲁土豪榜`）|
| `/帕鲁图鉴榜` | **图鉴收集排行**：按玩家已收集图鉴数排名（也可 `/帕鲁收集榜`、`/帕鲁图鉴收集榜`）|
| `/帕鲁闪光墙` | 全服**闪光帕鲁墙**（也可 `/帕鲁闪光`、`/帕鲁闪光帕鲁`、`/帕鲁幸运帕鲁`）|
| `/帕鲁头目墙` | 全服**头目帕鲁收集墙**（也可 `/帕鲁头目帕鲁`、`/帕鲁头目收集`）|
| `/帕鲁公会战力` | **公会战力榜**：按公会汇总成员帕鲁战力排名（也可 `/帕鲁公会战力榜`）|
| `/帕鲁帮助 [关键词]` | 指令帮助卡片（按类别分区，**数据驱动永远与实际指令同步**）；带关键词**搜指令**（如 `/帕鲁帮助 地图`）（也可 `/帕鲁菜单`）|
...
| `/帕鲁备份列表` | 查看存档**备份列表**（重置/回档产生的备份；也可 `/帕鲁备份`、`/帕鲁备份管理`）|
| `/帕鲁小队重置` | 重置本群**小队手动勾选清单**（存档读不到时的手动进度会被清空重来；也可 `squadreset`）|
...
| `/帕鲁词条查 <词条名> [编号]` | **词条查帕鲁**：列出你队伍/帕鲁箱里带这个词条的帕鲁（每只带**结果编号**）；再发 `/帕鲁词条查 <词条名> <编号>` 看列表里第几只的**完整详情**；「箱 #N」是箱内编号，也可 `/帕鲁箱查询 N` 单查（也可 `/帕鲁查词条`）|
...
> 🔐 **想校验完整性（可选）**：`main` 上的脚本会随更新变化，无法固定哈希；若需可校验的版本，改用**某个发布版 tag** 下载并核对其 Release 说明公布的 SHA256，例如 `v1.43.0`：
> ```bash
