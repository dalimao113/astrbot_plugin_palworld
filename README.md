...
![version](https://img.shields.io/badge/version-1.48.2-6366F1?style=flat-square)
...
支持 QQ 群查询、存档解析、图鉴攻略和服务器管理，并提供三主题卡片、自动维护预告与 Steam 官方中文更新公告。
**指令回复统一输出精美卡片图片**（基于 AstrBot `html_render`），仅更新公告等长文本使用合并转发，渲染失败时才发送兜底文字提示。
...
> 以下均为插件实际渲染的**奇幻玻璃**主题截图；另有**像素羊皮纸**与**游戏原生**主题，可在 WebUI 插件配置 `card_style` 切换。
...
| `/帕鲁塔主 [名]` | 8 大高塔塔主（已含盟约之塔“苍璃 & 霄龙”）：属性/等级/血量/**基础攻防/弱点属性/主要技能**/所在/掉落 + 克制提示（也可 `/帕鲁高塔`）|
| `/帕鲁突袭 [名]` | 突袭 Boss（贝菈诺娃等）：等级/血量/**基础攻防/弱点/技能**/掉落/获取方式与打法提示 |
| `/帕鲁boss [名]` | 汇总查询头目、高塔塔主与突袭 Boss；带名字查看详情（也可 `/帕鲁头目`）|
| `/帕鲁竞技场 [段位]` | 竞技场 6 段位（青铜→大师）对手阵容 + 段位首通/重复奖励；战斗券兑换见 `/帕鲁商人 竞技场商店` |
| `/帕鲁商人 [名]` | 10 个商店卖什么 + 价格/货币（村庄/流浪/沙漠/火山/勋章/赏金/竞技场/地牢…）|
| `/帕鲁哪里买 <物品>` | 某物品在哪个商店买、多少钱（数据来自客户端 pak 商店表）|
| `/帕鲁技能 <名/属性>` | 主动技能图鉴：威力/冷却/效果 + 是否技能果实可得（`/帕鲁技能 果实` 看全部果实）|
| `/帕鲁技能果实 [页]` | 1.0 技能果实图鉴：查看果实可学习的主动技能（也可 `/帕鲁果实图鉴`）|
...
| `/帕鲁获取 <物品>` | **完整获取来源查询**：汇总地图采集、帕鲁掉落、据点设施、制作配方、牧场、商店和钓鱼；特殊采集物直接附游戏物品图标与高清地图点位（也可 `/帕鲁来源`、`/帕鲁怎么获得`）|
| `/帕鲁矿点 [矿石]` | **矿石地点地图**：把当前游戏关卡中可直接投影的地表矿点标到主大陆/世界树游戏地图，保留原始世界坐标与独立地图坐标系；地下/实例层不冒充地表位置（也可 `/帕鲁矿石地图`、`/帕鲁矿脉`）|
| `/帕鲁解剖 [帕鲁/物品]` | **帕鲁解剖/解体查询**：按帕鲁看标准解体掉落，或按物品反查可解体哪些帕鲁；展示帕鲁/物品/切肉刀/解体传送带游戏图标并提示永久消失风险（也可 `/帕鲁解体`、`/帕鲁肢解`）|
...
| `/帕鲁地图收集 [类别]` | 以世界地图为底图，把**当前有可靠世界坐标的关卡对象**标到图上（编号+名称+坐标图例）：📍传送点（137）/🗿遗物雕像（140）/🕳️地牢入口（170）/禁猎区/遗迹遗址/聚落/洞窟/孤岛；无参给类别菜单（也可 `/帕鲁地标`）|
| `/帕鲁觉醒` | 1.0 **觉醒系统**与各属性觉醒材料说明（也可 `/帕鲁觉醒系统`）|
| `/帕鲁突变` | 1.0 **突变机制**与特殊蛋糕说明（也可 `/帕鲁突变配种`、`/帕鲁突变系统`）|
| `/帕鲁植入体 [名]` | **植入体图鉴**（1.0 改造系统）；无参给总览、带名给详情（也可 `/帕鲁改造`）；按编号查用 `/帕鲁植入体查询 <编号>`（也可 `/帕鲁植入体编号`）|
| `/帕鲁植入体查询 <编号>` | 按编号精确查询植入体（也可 `/帕鲁植入体编号`）|
| `/帕鲁世界树` | 1.0 世界树最终区域与剧情 Boss 专题（也可 `/帕鲁世界树boss`、`/帕鲁最终boss`）|
| `/帕鲁1.0` | Palworld 1.0 数据与插件支持总览（也可 `/帕鲁1.0总览`、`/帕鲁版本`）|
...
| `/帕鲁更新公告` | 直接发送 Steam 官方简体中文**完整更新公告**，长文使用合并转发，底部保留官方原文链接（也可 `/帕鲁更新日志`、`/帕鲁补丁说明`）|
| `/帕鲁战力榜` | 帕鲁**战力等级排行**（图鉴向，按种族基础战力；按游戏实际遭遇标注世界树/主线/塔主/突袭/野外/关押/地牢头目池等 Boss 类型；也可 `/帕鲁战力`、`/帕鲁最强帕鲁`）|
...
| `/帕鲁拒绝绑定 <QQ>` | 拒绝玩家绑定申请 |
| `/帕鲁解绑 <QQ/游戏名>` | 解除玩家角色绑定 |
| `/帕鲁存档` | 立即保存世界存档 |
| `/帕鲁关服 <0-300秒> [提示语]` | 先存档并公告，倒计时后明确停止 Docker 容器；不会被 `unless-stopped` 自动拉起（**需二次确认**） |
| `/帕鲁重启服务器` | 运行中会先存档再重启；已关服时直接启动容器。约 1 分钟恢复、世界不变（**需二次确认**；也可 `/帕鲁重启`） |
...
| `/帕鲁回档 <编号>` | 按备份列表编号回档；会先保存并另存当前世界，前置存档或安全备份失败则中止（**需二次确认**；也可 `/帕鲁回滚`） |
| `/帕鲁小队重置` | 重置本群**小队手动勾选清单**（存档读不到时的手动进度会被清空重来；也可 `squadreset`）|
| `/帕鲁确认` | 在超时时间内确认上一条危险操作 |

> **关服 / 重启 / 回档 / 重置 / 恢复存档**需容器挂载 `docker.sock`（与服务器负载监控同款挂载）；均为危险操作，走管理员白名单 + 二次确认。
> - `/帕鲁关服` 不再只退出 Palworld 游戏进程，而是由 Docker 明确停止容器；重新开服可发 `/帕鲁重启服务器`（检测到停止状态会执行 `start`），或在服务器面板手动启动。
...
| `/帕鲁小队进度` | 按当前群真实成员同步已绑定队员，无需在旧群解散后重新绑定；聚合**287 图鉴解锁/152 传送点/8 高塔/5 剧情 Boss/122 头目/407 遗物/123 区域**进度。全部按当前 build 稳定 ID 求交集，旧版本残留标记不再造成“全部完成”。目录或单项存档不可核验时只显示已记录数/存档不可用，不伪造总量、剩余和完成状态；地牢仅显示普通/固定累计清理次数及 170 个入口目录，无法计算逐入口完成度。|
...
| `admin_qq` | `[]` | 管理员 QQ 号列表 |
| `bind_mode` | `open` | 绑定模式：`open`=先到先绑；`admin_confirm`=需管理员批准 |
| `trusted_qq` | `[]` | `admin_confirm` 模式下可免批准自助绑定的受信任 QQ；管理员默认受信 |
...
| `broadcast_whitelist_only` | `false` | 仅向 `broadcast_groups` 白名单群播报，关闭自动登记 |
...
| `daily_reboot_time` | `""` | 每日重启时间；留空时自动识别 Palworld 容器的标准每日 cron，约提前 5 分钟向 QQ 预告 |
| `notify_server_update` | `true` | 精确核对服务端 depot manifest，差异稳定复核 60 秒后才通知 QQ、发送完整更新公告与维护前游戏公告 |
| `update_check_hours` | `1` | Steam 服务端版本检测间隔（小时，最小 1 小时；普通失败 10 分钟重试，计划更新窗口及服务器恢复后 1 分钟快速复核）|
| `backup_keep_max` | `0` | 每日 tar 备份保留份数；`0`=交给游戏服镜像按天清理 |
| `engine_backup_keep` | `30` | 游戏引擎 `backup/world/` 高频快照保留份数；`0`=不清理 |
...
| `force_save_min_interval` | `15` | 两次强制存盘的最小间隔秒数，避免频繁存盘影响服务器 |
| `save_neg_ttl` | `45` | 存档拉取/解析失败后的负缓存秒数（硬下限 10）|
...
- **例行维护预告**：`daily_reboot_time` 留空时只读识别 Palworld 容器的标准每日重启 cron；约提前 5 分钟向 QQ 预告并进入维护静默，避免把正常重启误报成掉线。
- **游戏更新预告**：默认每 1 小时精确比较 Steam 服务端 depot `2394012` 的远端与本地 manifest，临时失败会在 10 分钟后重试；发现差异后先等待 60 秒稳定复核，避免 `UPDATE_ON_BOOT` 每日重启重建 `appmanifest` 时产生误报。识别到镜像的每小时计划后，检测会自动对齐到更新倒计时前沿（默认整点检测、每小时第 5 分钟由镜像检查更新）。确认更新后才通知 QQ、发送 Steam 官方简体中文完整公告，并仅在计划分钟**之前**向在线玩家倒计时；部分群发送失败时 10 分钟后只补发失败群。实际更新和重启仍由镜像的 `AUTO_UPDATE_ENABLED` / `UPDATE_ON_BOOT` 负责，插件不会自行绕过确认重启 Docker。

这里的“游戏更新”是镜像内部通过 SteamCMD 更新 `/palworld` 中的 Palworld 服务端文件，不会自动拉取新的 Docker 镜像；Docker 镜像升级仍需显式执行部署脚本的 `--update` 或手动拉取/重新部署。
...
- **代码与数据分离**：数据放插件 `data/` 子目录，启动时加载，换数据文件不用改代码。
  ...
  - `data/breeding.json`：配种组合表（**34191 组**，按游戏当前 CombiRank 算法生成，覆盖 261 个可配种物种）。
...
- **前置条件**：把宿主机 docker socket 挂载进 AstrBot 容器，在 `docker-compose.yml` 的 astrbot 服务下加（示例保留 `:ro`，但它只限制 socket 文件节点，**不会限制 Docker API 写操作**）：
  ```yaml
  volumes:
...
## 卡片风格切换（皮肤）
插件内置三套正式卡片主题，当前模板键集合完全一致（各 71 个）：
- **🌌 奇幻玻璃（fantasy）**：二次元插画背景 + 深蓝紫磨砂玻璃 + 暗金宝石。**需你提供插画图**（见下）。
- **📜 像素羊皮纸（pixel）**：低像素 + aged 羊皮纸纹理 + 复古墨色（深红/金/暗绿）+ Fusion Pixel 中文像素字体。**离线自包含，不依赖 CDN 或外部图片**。
- **🎮 游戏原生（ingame）**：冷调蓝灰游戏 UI + 从客户端提取的面板、槽位与图标素材，全部卡片均为独立原生布局。

切换方式：在 **WebUI → 插件配置 → `card_style`** 选择 `fantasy`、`pixel` 或 `ingame`，保存后生效（如未即时生效可重载插件）。三主题共享属性、工作适性、货币等真实游戏语义图标；背景、面板、边框、字体和排版保持各自风格。

## 奇幻主题背景图
`fantasy` 主题使用**整图插画背景 + 深蓝紫磨砂玻璃信息面板 + 暗金复古描边 + 四角发光宝石**的现代手游 UI 风。
- **插画背景由你提供**（命名 `bg.jpg` 或每卡专属 `bg_<卡名>.jpg` 放插件目录）。建议主体偏右下、左上稍暗，玻璃面板压左上文字清晰、右下露出角色。
- 不放图时回退深蓝紫渐变背景，一切正常。
- 暗金/宝石/玻璃为 CSS 绘制（与 `card_theme_color` 无关，是固定奇幻配色）。

## 卡片输出
- 三主题均以手机竖屏为主，内容随长短自适应高度；部分队伍类卡片按内容加宽，游戏原生主题保持单列窄卡。
- **奇幻主题背景图（可每卡专属）**：把你有权使用的图片放进插件目录，卡片会显示右侧清晰图与左侧渐隐蒙版。
  - `bg.jpg`：**默认图**，所有卡片通用。
...
- **高清且控制体积**：默认 `SUPERSCALE=2` 超采样，远程 t2i 与本地渲染都使用 JPEG q90，避免长列表无损 PNG 达到 8～10 MB 后导致 QQ 上传超时。
- `card_theme_color` 控制支持该强调色的卡片元素（默认靛蓝 `#6366F1`）；三主题主体配色仍由各自模板决定。
...
| `render/` | Python | `templates.py`(fantasy / pixel / ingame 三主题、各 71 个模板) + `renderer.py`(JPEG q90 渲染引擎) + `assets.py`(游戏素材解析) |
...
| `data/breeding.json` | JSON 数据 | 配种组合表（34191 组，261 个可配种物种）|
...
| `data/worldmap.png` · `worldmap_hd.jpg` | 图片 | 主大陆 8192² 游戏地图底图（用于在线地图、栖息区域、矿点与地标）|
| `data/treemap_hd.jpg` | 图片 | 世界树 8192² 独立地图底图（使用独立坐标变换）|
| `data/map_*.json` · `regions.json` | JSON 数据 | 地图坐标变换 / 区域 / 传送点锚点 |
| `data/progress_catalog.json` | JSON 数据 | 小队进度 schema v2：当前 build 的图鉴、传送点、高塔、剧情 Boss、头目、12 类遗物与区域稳定 ID；地牢仅记录入口目录与累计计数口径 |
| `data/resource_nodes.json` | JSON 数据 | 特殊采集物/矿石的主大陆与世界树原始世界坐标、采集条件、刷新和单点产量（Palworld 1.0 `buildid 24181105`）|
| `data/images/` | 图片目录 | 全部图标：帕鲁头像 + `items/`(物品) + `buildings/`(设施) + `tech/`(科技) |
| `palwork/palsave.py` | Python | 存档解析器（解析 PlM/Oodle 存档格式，读取个人档案/背包/队伍/帕鲁箱/公会）|
...
`POST /v1/api/announce`、`/kick`、`/ban`、`/unban`、`/save`。
全部走 HTTP Basic Auth（`admin` + `ADMIN_PASSWORD`），请求超时默认 5s，异常回离线/错误卡片。

> Palworld 官方也提供 REST `/shutdown` 与 `/stop`，但它们只停止游戏服务进程；容器配置了自动重启策略时可能立即被拉起。因此插件的 `/帕鲁关服` 会在 REST 存档/公告成功后改走 Docker `stop`，确保保持关服状态。
...
**存档功能的前置条件：**
1. **挂载 docker.sock**：AstrBot 容器读帕鲁服容器的存档 + 负载，需按上文 `volumes` 示例把宿主 docker socket 挂进 AstrBot 容器；即使写 `:ro` 也仍可调用 Docker 写 API。
...
> 挂载 `docker.sock` ≈ 赋予该容器近乎宿主机 **root** 的能力。`:ro` 只让 socket 文件节点只读，**不会阻止 Docker API 的 stop / exec / create 等写操作**。请确保 AstrBot 本身可信、`admin_qq` 白名单配置正确。所有经 socket 的操作集中在 `api/docker_api.py`，高危写操作（关服/重启/删档/回档/exec/helper 容器）有 `[高危]` 注释并受白名单 + 二次确认保护。
...
| **运维模式** | 挂载（示例使用 `:ro`，仍是宿主 root 级能力） | 额外启用 容器负载、存档解析（背包/队伍/我/公会）、关服/重启/回档/重置/恢复 |
