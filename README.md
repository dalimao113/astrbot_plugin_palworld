...
![version](https://img.shields.io/badge/version-1.24.0-6366F1?style=flat-square)
...
| `/帕鲁地图收集 [类别]` | 以世界地图为底图，把**带坐标的探索/采集点**（传送点/禁猎区/遗迹遗址/聚落/洞窟/孤岛）标到图上（编号+名称+坐标图例，翻页）；无参给类别菜单（也可 `/帕鲁地标`）。注：游戏文件无单个雕像/遗物宝箱逐点坐标，只标有坐标的命名地标 |
| `/帕鲁帮助` | 指令帮助卡片（也可 `/帕鲁菜单`）|
